...
def refish(jsonfile, csvfile = "lexicon.tsv", fstfile = "refishing-fst.txt"):
    ...
    return {'columns': input_columns, 'boards': json_boards, 'currentBoard': json_current_board}


## For running the file as a program:
# Parse arguments with argparse
parser = argparse.ArgumentParser(description='Refish a collection of cognate sets with a new set of phonological histories.', add_help=True)
...
parser.add_argument('fstfile', metavar='fst_file', help='New transducers for refishing purposes')
args = parser.parse_args()

json.dump(refish(args.jsonfile, args.csvfile, args.fstfile), sys.stdout, indent=2)